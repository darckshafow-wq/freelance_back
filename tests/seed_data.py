...
def seed_data():
    ...
    try:
        ...
        # ========== LOCATIONS (Default) ==========
        print("\n🌍 Creating default location (Côte d’Ivoire)...")
        from app.models.location import Country, City, District
        country = Country(id=1, name='Côte d’Ivoire', code='CI')
        db.merge(country)
        city = City(id=1, name='Abidjan', country_id=1, latitude=5.3, longitude=-4.0)
        db.merge(city)
        district = District(id=1, name='Zone 4', city_id=1)
        db.merge(district)
        db.commit()
